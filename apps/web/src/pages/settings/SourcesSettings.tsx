...
export default function SourcesSettings() {
  ...
  if (error) return <ErrorBanner message={t("settings.sources.error")} onRetry={() => mutate()} />;
        ...
        <div className="overflow-x-auto rounded-[var(--radius-lg)] border border-[var(--color-neutral-200)]">
          <table className="w-full text-[var(--text-body-size)]">
            <thead className="bg-[var(--color-neutral-50)] border-b border-[var(--color-neutral-200)]">
              <tr>
                <th className="px-[var(--space-4)] py-[var(--space-3)] text-left font-[var(--text-caption-weight)] text-[var(--text-caption-size)] text-[var(--color-neutral-500)] uppercase tracking-wider">{t("settings.sources.table.name")}</th>
                <th className="px-[var(--space-4)] py-[var(--space-3)] text-left font-[var(--text-caption-weight)] text-[var(--text-caption-size)] text-[var(--color-neutral-500)] uppercase tracking-wider">{t("settings.sources.table.type")}</th>
                <th className="px-[var(--space-4)] py-[var(--space-3)] text-left font-[var(--text-caption-weight)] text-[var(--text-caption-size)] text-[var(--color-neutral-500)] uppercase tracking-wider">{t("settings.sources.table.status")}</th>
                <th className="px-[var(--space-4)] py-[var(--space-3)] text-left font-[var(--text-caption-weight)] text-[var(--text-caption-size)] text-[var(--color-neutral-500)] uppercase tracking-wider">{t("settings.sources.table.lastSync")}</th>
              </tr>
            </thead>
            <tbody>
              {sources.map((src, i) => (
                <tr key={src.id} className={i % 2 === 1 ? "bg-[var(--color-neutral-50)]" : ""}>
                  ...
                  <td className="px-[var(--space-4)] py-[var(--space-3)] text-[var(--color-neutral-500)]">
                    {src.last_sync_at ? new Date(src.last_sync_at).toLocaleString() : t("settings.sources.neverSynced")}
                  </td>
                </tr>
              ))}
            </tbody>
